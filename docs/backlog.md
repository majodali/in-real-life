...
- [ ] Evidence-based decay (ranking spec v6, `docs/evidence-decay.md` — proposal awaiting sign-off) — replace the three clock-based half-lives with the activity axis (decay per debriefed event, min-of-both-sides for confirmed strength), decay floors as the incompleteness prior (established ties never fade below ~half on silence alone; below-floor reserved for grounded counter-evidence — D49 avoidance, D50 blocks), met-without-tap captured-not-used (normalization is indistinguishable from cooling). Philosophy recorded in the note + D59; watch signals: returning-member reconnection, floor saturation, stale-steer pressure (routes to prioritizing D49, never to silent re-clocking)
