...
These notes are **living and provisional** — current best decisions, revisable as real usage teaches us. `decisions.md` is the map: a register of every decision (D1–D48) with a one-line pointer to the note that holds the reasoning, plus watch-items for emergent effects we're tracking.
...
**Start:** `decisions.md` (the decision register) · `hypotheses.md` (the hypothesis register — every embedded analysis as a testable, tunable, reversible hypothesis) · `open-risks.md` (known gaps) · **`flows.md` (the visual map — all processes and flows as diagrams, TBD elements dashed)** · this README.
