...
These notes are **living and provisional** — current best decisions, revisable as real usage teaches us. `decisions.md` is the map: a register of every decision (D1–D61) with a one-line pointer to the note that holds the reasoning, plus watch-items for emergent effects we're tracking.
...
- `matching-spec.md` — the implemented, versioned ranking spec (v7): tunables + defaults, hard constraints, the honest register of what it omits and where each piece lands.
...
- `localities-and-constraints.md` — **(proposal, revised)** the curated locality register (effort bands — here/nearby/a-trip/far — no coordinates) and structured constraints: travel reach as a graduated de-weight (prioritization, never filtering), time windows as soft fit, both member-correctable; effort-is-personal watch signals.
