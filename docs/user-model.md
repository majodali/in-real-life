...
Users can name people they do not want to interact with. This is the **comfort tier** — "I'd rather not share a table with them" — distinct from a **protective block** (D50, Group 4), the safety tier. **The line between the tiers is drawn by need, never by relationship type**: "ex-spouse" can be an awkwardness or a danger, and a member must never have to prove which. When the need is safety — stalking, abuse, fear — the right tool is the block: hard, immediate, proof-free, silent, presence-shielding (D50); capture-time routing must make that path unmistakable without demanding disclosure or justification. Like every barrier, comfort-tier avoidance **does not reduce either person's visibility of events** — both can still see and attend anything.
...
**Protective blocks invert the symmetry — deliberately (D50).** Anti-observation for the comfort tier means *neither* party gains observational power. A block is asymmetric in the victim's favour: the blocker's name, attendance, and existence are shielded from the blocked person absolutely, on every surface; the blocked person's *event* visibility is never conditionally altered (unconditional invisibility destroys the differential rather than encoding it — there is no negative-space signal to read); and the blocked person gains nothing and is never notified. What the *blocker* may see of the blocked — e.g. to avoid walking into a room with them — was the hard question, now resolved in **`protective-blocks.md`** (D52): the blocker keeps **ordinary member power only** (no flags, no aggregation), with a capture-time awareness / peace choice over their own view.
