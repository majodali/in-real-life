...
| T10 | AWS region and credentials are resolved, never defaulted: explicit env → the active profile's configured region → a loud failure (`scripts/inject-config.mjs`, `test/helpers/region.mjs`) | A hardcoded `us-east-1` fallback survived the move to us-west-2 and made "stack does not exist" the default failure — a wrong region is indistinguishable from a missing deploy, so guessing costs debugging time; errors now name the region and profile in use | `test/helpers/region.mjs`, `scripts/inject-config.mjs` |
| T11 | The service worker caches the app shell only — never API responses; cross-origin requests are not intercepted at all, and navigations are network-first with the cached shell as offline fallback | The worker exists so the site can be installed (and, later, receive push — iOS allows web push only for installed web apps); it is not an offline-data feature. A stale roster or event time is worse than a spinner: someone could travel to a cancelled event on cached data. Asserted by test | `src/sw.js`, `design-spec.md` §5 |
